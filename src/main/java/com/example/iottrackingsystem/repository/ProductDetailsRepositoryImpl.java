...
//            productListModel = new ProductListModel();
            ...
            for (int i = 1; i < records.size(); i++) {
                ...
                product.setDateTime(Long.valueOf(records.get(i).get(0)));
...
//            records .stream().forEach((x) -> x.stream().forEach(System.out::println));
    ...
    public DeviceReportResponseDTO getDeviceAndLocationDetails(String productId, Long tstmp) {
        ...
        Product product = dataService.getProductDetails(productId, tstmp.toString());
        ...
        responseDTO.setDatetime(Utils.convertToUTCDatetime(product.getDateTime().toString()));
        ...
        if(product.getAirplaneMode().toUpperCase().equals("OFF")){
            ...
            responseDTO.setLongi(product.getLongitude());
                ...
                responseDTO.setStatus(IotTrackingSystemConstants.FLAG_ACTIVE);

        }
        else{
            responseDTO.setDescription("SUCCESS: Location not available: Please turn off airplane mode.");
            responseDTO.setStatus(IotTrackingSystemConstants.FLAG_INACTIVE);
        }
        ...
        if(batteryLife >= 98)
            responseDTO.setBattery(IotTrackingSystemConstants.BATTERY_FULL);
        else if (batteryLife >= 60 && batteryLife < 98)
            responseDTO.setBattery(IotTrackingSystemConstants.BATTERY_HIGH);
        else if (batteryLife >= 40 && batteryLife < 60)
            responseDTO.setBattery(IotTrackingSystemConstants.BATTERY_MEDIUM);
        else if (batteryLife >= 10 && batteryLife < 40)
            responseDTO.setBattery(IotTrackingSystemConstants.BATTERY_LOW);
        else if (batteryLife < 10)
            responseDTO.setBattery(IotTrackingSystemConstants.BATTERY_CRITICAL);
